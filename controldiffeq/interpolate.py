...
def natural_cubic_spline_coeffs(t, X):
    ...
    Returns:
        ...
        (c) Save the result.
        (d) Treat the result as your datasets as far as PyTorch's `torch.utils_kovae.datasets.Dataset` and
            `torch.utils_kovae.datasets.DataLoader` classes are concerned.
...
class NaturalCubicSpline:
    ...
    def __init__(self, times, coeffs, **kwargs):
        ...
        # print(a.shape,b.shape,two_c.shape,three_d.shape,times.shape)#torch.Size([64, 23, 6]) torch.Size([64, 23, 6]) torch.Size([64, 23, 6]) torch.Size([64, 23, 6]) torch.Size([24])
        # time.sleep(500)
        self._times = times
        self._a = a
        self._b = b
