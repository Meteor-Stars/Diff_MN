...
class TimeDataset_irregular(torch.utils.data.Dataset):
    def __init__(self, seq_len, data_name, missing_rate=0.0,args=None):
        ...
        loc = PROJECT_DIR / 'datasets' / (data_name + str(missing_rate)+'-'+str(args.seq_len))

        if os.path.exists(loc) and os.path.isdir(loc) and os.listdir(loc) and data_name!='polynomial' and data_name not in args.medical_datasets:
            print('Loading data from cache')

            tensors = load_data(loc,args)
            ...
            if data_name in ['stock', 'energy']:
                ...
                path='./datasets/HopperPhysics/training_{}.pt'.format(args.seq_len)
                data = torch.load(path,weights_only=False)
                norm_data = MinMaxScaler(data)
                # print(norm_data.shape)
            elif data_name == 'sine':
                norm_data = sine_data_generation(no=10000, seq_len=args.seq_len, dim=5)
                norm_data=np.concatenate(norm_data,axis=0)


            elif data_name=='polynomial':
                data_all=generate_polynomial_dataset(args.n_samples,args.seq_len)
                X=data_all['X']
                Y=data_all['Y']

                np.save(args.log_dir+'/'+'polynomial_X_gt.npy',X)
                np.save(args.log_dir + '/' + 'polynomial_Y_gt.npy', Y)
                norm_data=np.expand_dims(Y,axis=-1)
            ...
            if data_name in ['stock', 'energy']:
                ...
                # print(norm_data.shape)#(3685, 6)
                norm_data = np.concatenate((norm_data, time), axis=1)
                # print(norm_data.shape)#(3685, 7)
                ...
                norm_data_tensor = torch.Tensor(self.samples[:, :, :-1]).float().to(self.device)

            else:
                self.original_sample=copy.deepcopy(norm_data)

                generator = torch.Generator().manual_seed(SEED)
                num_samples, num_timesteps, num_vars = norm_data.shape
                for i in range(num_samples):

                    removed_timesteps = torch.randperm(num_timesteps, generator=generator)[
                                        :int(num_timesteps * missing_rate)].sort().values

                    norm_data[i, removed_timesteps, :] = float('nan')
            ...
            if data_name!='polynomial' and data_name not in args.medical_datasets:
